    ...
    if( method !== undefined ) {
      ...
      try {
        ...
        context.res._send = function modifiedSend( err, rv ) {
          console.log( 'modifiedSend', err, rv );

          var args = [].slice.call( arguments );

          if( args.length === 1 ) {
            return send( context, new XmlRpcResponse( [rv] ).xml() );
          }

          if( !err ) {
            send.call( context, new XmlRpcResponse( [rv] ).xml() );
          } else {
            send( context, new XmlRpcFault( err.code || 0, err.message || err ).xml() );
          }
