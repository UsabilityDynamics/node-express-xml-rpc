...
XmlRpcParser.prototype.getHandler = function getHandler() {
  ...
  return function( cb ) {
    ...
    cb.onEndElementNS( function( elem, prefix, uri ) {
      var cdata = instance.cdataStack.pop().join( '' ).trim(), last_elem = instance.elementStack.pop();
      switch( elem ) {
        ...
        case 'member':
          var curr_name = instance.nameStack.pop(), curr_value = instance.valueStack.pop();
...
function decodeBase64( sEncoded ) {
  ...
  for( i = 0; i < sEncoded.length; i += 4 ) {
    nBits = (base64.indexOf( sEncoded.charAt( i ) ) & 0xff) << 18 | (base64.indexOf( sEncoded.charAt( i + 1 ) ) & 0xff) << 12 | (base64.indexOf( sEncoded.charAt( i + 2 ) ) & 0xff) << 6 | base64.indexOf( sEncoded.charAt( i + 3 ) ) & 0xff;
...
function parseISO8601( string ) {
  ...
  var time, offset = 0, dOut = new Date(), d = string.match( new RegExp( regexp ) ), date = new Date( d[1], 0, 1 );
